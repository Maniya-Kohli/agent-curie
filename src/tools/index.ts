...
// Core tool definitions (non-memory)
const CORE_TOOL_DEFINITIONS: Tool[] = [
  {
    name: "get_weather",
  ...
  },
  {
    name: "create_event",
    description:
      "Create a new event in Google Calendar. Use ONLY for calendar events (meetings, appointments, scheduled blocks). Do NOT use for reminders or todos — use the reminders skill instead.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "system_exec",
    description:
      "Execute a shell command on the host Mac. Allowed commands: osascript, shortcuts, open, say. Use for macOS integrations like Apple Reminders, Notes, Shortcuts, and system controls.",
    input_schema: {
      type: "object",
      properties: {
        command: {
...
// Core tool functions
const CORE_TOOL_FUNCTIONS: Record<string, Function> = {
